    ...
    for (let i=0;i<this.notePos.length;i++){
      for (let j=0;j<this.notePos[i].length;j++){
        ...
        if (this.noteState[i][j]>0) {
          ...
          let c1 = color(255,200, 80,alpha);
          let c2 = color(220, 50, 120,alpha);
          let mix = lerpColor(c1, c2, map(i, 0, this.notePos.length, 0, 1));
          fill(mix);
          let s = this.noteState[i][j];
          let angle = noise(x, y) * PI;
          for (let k = 0; k <= 360; k += 30) {
            translate(frameCount * sin(angle) / 2, frameCount * cos(angle) / 2);
            rotate(k);
            ellipse(x + random(10, 50), y + random(10, 50), this.noteSize*s, this.noteSize*s);
            rect(x + random(20, 70), y + random(10, 50), this.noteSize*s, this.noteSize*s);
          }
        }
